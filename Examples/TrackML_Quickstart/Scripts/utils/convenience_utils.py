...
def get_training_metrics(trainer):
    ...
    metrics = pd.merge(left=train_metrics, right=val_metrics,
                       how='inner', on='epoch')
...
def plot_training_metrics(metrics):
    ...
    for idx, y in enumerate(['train_loss', 'val_loss']):
        p1.circle(y=y, x='epoch', source=source,
                  color=cmap[idx], legend_label=y)
        p1.line(x='epoch', y=y, source=source, color=cmap[idx], legend_label=y)

    p2 = figure(title='Purity on validation set',
                x_axis_label='Epoch', y_axis_label='Purity')
    p2.circle(y='pur', x='epoch', source=source,
              color=cmap[0], legend_label='Purity')
    p2.line(x='epoch', y='pur', source=source,
            color=cmap[0], legend_label='Purity')

    p3 = figure(title='Efficiency on validation set',
                x_axis_label='Epoch', y_axis_label='Efficiency')
    p3.circle(y='eff', x='epoch', source=source,
...
def plot_neighbor_performance(model):
    ...
    for idx, y in enumerate(['eff', 'pur', 'loss']):
        ...
        figures[-1].circle(y=y, x=x, source=source,
                           color=cmap[0], legend_label=y)
        figures[-1].line(x=x, y=y, source=source,
                         color=cmap[0], legend_label=y)
        y_val = results[y][(
            results[x] - model.hparams["r_test"]).abs().idxmin()].item()
...
def plot_predicted_graph(model):
    ...
    p = figure(title='Truth graphs', x_axis_label='x',
               y_axis_label='y', height=500, width=500)
    q = figure(title='Predicted graphs', x_axis_label='x',
               y_axis_label='y', height=500, width=500)
...
def plot_track_lengths(model):
    ...
    p1 = figure(title='Histogram of true track lengths',
                x_axis_label='Edges', y_axis_label='Count', height=400, width=400)
    p2 = figure(title='Histogram of predicted track lengths',
                x_axis_label='Edges', y_axis_label='Count', height=400, width=400)
    p1.quad(bottom=0, top='true_hist', left='low', right='high',
    ...
    with torch.no_grad():
        for data in tqdm(model.trainset):
            results = model.shared_evaluation(
                data.to(device), 0, 0.12, 100, log=False)
...
def plot_edge_performance(model):
    ...
    with torch.no_grad():
        ...
        auc = roc_auc_score(
            test_results["truth"].cpu(), test_results["score"].cpu())
